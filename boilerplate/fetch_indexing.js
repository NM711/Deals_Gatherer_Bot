...
const { interactionUpdate, interactionReply } = require('../components/intraction_methods.js')
...
//  I know, its bad even im dissapointed. Will try to keep clean while moving forward :(
module.exports.glob_indexing = async (interaction, endpointName) => {
  ...
  const reply = await interactionReply(interaction)({ embeds: paginated(), components: [announce()], ephemeral: true })
  ...
  collector.on('collect', async (interaction) => {
    if (interaction.customId === 'load_more' && interaction.user.id) {
      ...
      if (paginated().length !== 0) {
        await interactionUpdate(interaction)({ embeds: paginated(), components: [announce()] })
      } else await interactionUpdate(interaction)({ embeds: [no_more_items()], components: [announce()] })
